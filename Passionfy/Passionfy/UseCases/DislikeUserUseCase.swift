...
struct DislikeUserUseCase {
    ...
    func execute(params: DislikeUserParams) async throws -> Void {
        guard let currentUserId = try await authRepository.getCurrentUserId() else {
            throw DislikeUserError.dislikeFailed
        }
        do {
            try await userRepository.dislikeUser(userId: currentUserId, targetUserId: params.targetUserId)
        } catch {
            throw DislikeUserError.dislikeFailed
        }
    }
